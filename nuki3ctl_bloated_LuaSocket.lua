...
if not ok_dkjson or not ok_http then
    print("The packages/modules dkjson + luasocket (<400 KB) are required to run.")
    os.exit(1)
end
local os = require 'os'
local io = require 'io'
...
if action == 'list' then
    ...
    if code ~= 200 then
        print("Error listing devices: HTTP " .. code)
        os.exit(1)
    end
...
local body, code = http.request(url)
if code ~= 200 then
    print("Error performing " .. action .. ": HTTP " .. code)
    os.exit(1)
end
